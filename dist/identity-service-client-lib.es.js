...
//#region src/types/global.ts
var l = /* @__PURE__ */ function(e) {
	return e.UPDATE_EMAIL = "update_email", e.UPDATE_PASSWORD = "update_password", e;
}({}), u = async (e, t) => {
...
async function p() {
	...
	return {
		verifier: e,
		challenge: f(await crypto.subtle.digest("SHA-256", t))
	};
}
//#endregion
//#region src/utils/storage.ts
var m = "identity_service_pkce_verifier", h = "identity_service_state", g = "identity_service_required_actions";
function _(e) {
...
function w() {
	let e = localStorage.getItem(g) || "null";
...
function k(e, t) {
	if (typeof e != "string") throw new E("Invalid token specified: must be a string");
	t ||= {};
	let n = t.header === !0 ? 0 : 1, r = e.split(".")[n];
	if (typeof r != "string") throw new E(`Invalid token specified: missing part #${n + 1}`);
	let i;
	try {
		i = O(r);
	} catch (e) {
		throw new E(`Invalid token specified: invalid base64 for part #${n + 1} (${e.message})`);
	}
	try {
		return JSON.parse(i);
	} catch (e) {
		throw new E(`Invalid token specified: invalid json for part #${n + 1} (${e.message})`);
	}
}
//#endregion
...
var j = "netapp_identity_channel", M = class {
	channel = new BroadcastChannel(j);
	publish(e) {
	...
	setAccessToken(e) {
		...
		this.tokenDecoded = A(e), this.scheduleRefresh(this.tokenDecoded.exp * 1e3);
	}
	scheduleRefresh(e) {
		this.clearRefreshTimer();
	...
	async login() {
		let { verifier: e, challenge: t } = await p(), n = crypto.randomUUID();
		_(e), b(n);
	...
	async loginRequest(e, t) {
		let { verifier: n, challenge: r } = await p(), i = crypto.randomUUID(), a = await fetch(`${this.options.identityUrl}/auth/sign-in`, {
			method: "POST",
			credentials: "include",
			headers: { "Content-Type": "application/json" },
			body: JSON.stringify({
		...
		if (!a.ok) throw Error("Unexpected error");
		let o = await a.json();
		if (o.requiredActions.length) {
			let e = o.requiredActions.includes(l.UPDATE_PASSWORD), t = {
				email: o.requiredActions.includes(l.UPDATE_EMAIL),
				password: e
			};
			C(t), this.requiredActions = t;
		}
		this.setAccessToken(o.accessToken);
	}
	async handleCallback() {
		...
		if (!n || !r) return "check";
		if (r !== x()) throw Error("Invalid state");
		let i = v();
		...
		let o = await a.json();
		this.setAccessToken(o.accessToken), y(), S(), globalThis.history.replaceState({}, document.title, globalThis.location.pathname);
		let s = o.accessToken ? "success" : "error";
		if (s === "success") {
			let t = e.get("updatepassword") === "true", n = e.get("updateemail") === "true";
			if (t || n) {
				let e = {
					email: n,
					password: t
				};
				C(e), this.requiredActions = e;
			}
		}
		return s;
	}
	...
	async updateSpecificFields(e, t, n) {
		...
		return T(), this.requiredActions = N, i;
	}
}, F = /^[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}$/i, I = ({ email: e, password: t, onSubmit: n }) => {
	let [o, l] = a({
		mail: "",
		pass: "",
		cpass: ""
	...
	return /* @__PURE__ */ c("form", {
		...
		children: [
			/* @__PURE__ */ s("p", {
				...
				children: "Antes de continuar, necesitamos que actualices la siguiente información:"
			}),
			e ? /* @__PURE__ */ c("div", {
...
function V(e) {
	...
	return t.interceptors.request.use(async (e) => await u(e, B)), t.interceptors.response.use((e) => e, async (e) => t(await d(e, B))), t;
}
var H = t({}), U = () => n(H), W = (t) => {
	let { appName: n, options: o, global: l, children: u } = t, [d, f] = a(!0), [p, m] = a(!1), [h, g] = a(!1), [_, v] = a(!1), y = async () => {
		try {
			if (f(!0), h) return;
			g(!0), B = new P(o);
			let e = await B.handleCallback();
			if (e === "error") {
				v(!0);
				return;
			}
			e === "check" && await B.restoreSession(), m(B.isAuthenticated());
		} finally {
			if (h) return;
			g(!1), f(!1);
		}
		...
		try {
			if (!x.tokenDecoded?.sub) throw Error("NOT FOUND USER ID");
	...
	return /* @__PURE__ */ s(H.Provider, {
		value: x,
		children: d ? /* @__PURE__ */ s(z, {
			name: n,
			children: /* @__PURE__ */ s("img", {
				src: L,
				alt: "loader"
			})
		}) : /* @__PURE__ */ s(e, { children: p ? /* @__PURE__ */ s(e, { children: S?.email || S?.password ? /* @__PURE__ */ s(z, {
			name: n,
			children: /* @__PURE__ */ s(I, {
				...S,
				onSubmit: b
			})
		}) : /* @__PURE__ */ s(e, { children: u }) }) : /* @__PURE__ */ s(e, { children: _ ? /* @__PURE__ */ s(z, {
			name: n,
	...
	children: /* @__PURE__ */ c("div", {
		...
		children: [/* @__PURE__ */ s("h1", {
			...
			className: "sso__paragraph",
			children: ["No se obtuvo la información del usuario desde el ", /* @__PURE__ */ s("b", { children: "servidor interno" })]
		})]
	})
});
//#endregion
